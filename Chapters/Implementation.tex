...
\begin{figure*}
	...
	\caption{Gráfico comparando os resultados da energia por partícula com o programa em Fortran com o em programa em C. \protect\input{graphics/Comp_C_F/info.tex}}
	\label{Fig:energy_Comp_C_F}
\end{figure*}

\begin{figure*}
	\input{graphics/Comp_C_F/graph/pressure.tex}
	\caption{Gráfico comparando os resultados da pressão com o programa em Fortran com o em programa em C. \protect\input{graphics/Comp_C_F/info.tex}}
...
\begin{itemize}
	\item NJL:
		\begin{itemize}
			\item Descobrir de onde as expressões usadas no código em Fortran vêm. Dessa forma eu posso verificar todos os detalhes e saber se há alguma explicação para as alterações que eu tive que fazer para poder fazer funcionar (pelo que lembro teve um dois multiplicando que entrou ou saiu). Além disso, fico com mais confiança no que estou fazendo.
	...
	\item Como fica a adição de temperatura para tudo isso? Muda alguma coisa para a equação do gap? Acredito que sim, pois pelo menos no caso eNJL temos dependência em $\rho_s$, o que implica em uma dependência indireta em $T$ (eu acho).
	\item Como determinar quais são as variáveis do loop?
	\item Cálculo de acordo com Buballa\cite{Buballa1996}
		\begin{itemize}
			\item [OK] Reler artigo
			\item Ver a integral da eq. 2.4, deve ser de $0 \to p_F$, não de $p_F \to \Lambda$; Se funcionar, alterar as notas no cap. de Termodinâmica.
			\item Reescrever com loop em $\mu$;
			\item Reescrever para $G_V$ não nulo;
		\end{itemize}
\end{itemize}
