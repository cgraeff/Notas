...
\end{equation}
%
Here $\psi$ represents the quark field, $m_0$ the quark bare mass, and $G_S$ and $G_V$ are coupling constants that are chosen so that the model reproduces \comment{stuff}. 
...
\end{equation}
%
Here the renormalized chemical potential $\mu_R$ and the constituent mass $m$ are given by
\begin{align}
	\mu_R &= \mu - 2 G_V \mean{\psi^\dagger\psi} \\
	m &= m_0 - 2 G_S \mean{\bar\psi\psi}
\end{align}
%
and
\begin{equation}
...
\comment{Essas expressões devem ser particularizadas para $T = 0$ aqui.}

\comment{Comentários sobre a solução das equações (gap, variável de loop?)} 
